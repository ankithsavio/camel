...
#     system_message = """You are a Document Processing Assistant specialized
# in
...
# #     - Provide clear feedback about document creation and modification
# # processes
# #
...
# #     documents with professional quality and appropriate formatting across
# all
...
async def main(rerun: bool):
    task_dict = {
        "task_id": "0a3cd321-3e76-4622-911b-0fda2e5d6b1a",
        "Question": "According to the World Bank, which countries had gross "
        "savings of over 35% of GDP for every year in the period 2001-2010? "
        "Give your answer as a comma-separated list of countries in "
        "alphabetical order. Use the countries most common names in english "
        "when answering.",
        "Level": 2,
        "Final answer": "Brunei, China, Morocco, Singapore",
        "file_name": "",
        "Annotator Metadata": {
            "Steps": "1. Use search engine to search for \"World Bank gross "
            "savings % of GDP\".\n2. Open World Bank data webpage showing "
            "gross"
            " savings as % of GDP (https://data.worldbank.org/indicator/"
            "NY.GNS.ICTR.ZS).\n3. Download data from webpage as Excel file and"
            " open it in a spreadsheet editor like Microsoft Excel.\n4. Go to "
            "the file's \"Data\" sheet.\n5. Add columns with formulas "
            "indicating if the gross savings % of GDP figures in each of the "
            "years from 2001 to 2010 are greater than 35 for each row.\n6. "
            "Add column computing AND of the boolean values from the previous "
            "step for each row.\n7. Filter for rows where the output of the "
            "AND from the previous step is true.\n8. Get the list of country "
            "names in the remaining rows, excluding non-country regions and "
            "categories.\n9. Sort the list alphabetically and format it as a "
            "comma-separated list to get the final answer: Brunei Darussalam, "
            "China, Morocco, Singapore",
            "Number of steps": "9",
    ...
    if not rerun:
        case_studies_path_prefix = "case_studies/" + datetime.today().strftime(
            "%d_%m_%y_"
        )
        with open(
            case_studies_path_prefix
            + task_dict["task_id"]
            + "/task_metadata.json",
            "w",
        ) as file:
        ...
        log_path = case_studies_path_prefix + "/eigent_logs.json"
        workforce.dump_workforce_logs(log_path)


if __name__ == "__main__":
    rerun = False
    asyncio.run(main(rerun))
